...
const ServicesSection = () => {
  ...
  const fetchServices = async (
    pageUrl = `${BASE_URL}services?page=${currentPage}`
  ) => {
    const params = {
      dataLimit: 3,
    };
  ...
  return (
    <section
      className={`pb-12 bg-[#2482ce23] mt-[64px] pt-8 px-4 `}
    ...
    >
      <div className="text-center mb-8">
        ...
        <p className="mt-2 xl:text-2xl lg:text-2xl md:text-2xl text-xl font-semibold text-[#2482ced8] mb-4">
          {t("services.services_subtitle")}
        </p>
      </div>
      <div className="max-w-[85rem] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-9 min-h-[200px] my-8 ">
        {services?.length ? (
            ...
            >
              ...
              {service?.is_free ? (
                <div className="absolute top-6 -left-12 bg-green-500 text-white font-bold px-2 py-[6px] text-lg transform rotate-[-45deg] shadow-lg w-[200px] text-center">
                  {t("services.service_free")}
                </div>
              ) : (
                <div className="absolute top-6 -left-12 bg-gradient-to-r from-[#2482ced8] to-[#1e6bb8] text-white font-bold px-2 py-[6px] text-lg transform rotate-[-45deg] shadow-lg w-[200px] text-center">
                  {service?.price - (service?.price * service?.discount) / 100}{" "}
                  {t("common.currency")}
                </div>
              )}
              <div className="absolute top-6 -right-12 bg-gradient-to-r from-[#2482ced8] to-[#1e6bb8] text-white font-bold px-2 py-[6px] text-lg transform rotate-[45deg] shadow-lg w-[200px] text-center line-through">
              ...
              <p
                className="px-6 text-slate-800 font-bold mt-6 mb-3 text-2xl"
                onClick={() => openModal(service)}
              >
                {service?.name}
              </p>
              <p
                className="flex px-6 text-gray-500 min-h-[160px] detilas-p hover:opacity-75 cursor-pointer text-justify"
                onClick={() => openModal(service)}
              >
                {service?.details}
              </p>
              <div className="flex justify-between items-center px-6 py-[14px] text-main">
